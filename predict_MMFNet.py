...
test_imgs_path = '/media/brainplan/XLdata/CNTSeg++/Data/NewDataset/ValSet'
# test_extraction_step = config_2d.TEST_EXTRACTION_STEP
# 是否使用cuda
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1,test_imgs_path,fold):  #
    ...
    for test_num in test_dir:
        ...
        test_dataset = CN_MyTrainDataset(CN_test_x_t1_dir, CN_test_x_t2_dir, CN_test_x_fa_dir, CN_test_x_dec_dir,
                                         ...
                                         CN_test_y_4_dir, x_transform=x_transforms,
        ...
        with torch.no_grad():
            for x1, x2, x3, x4, x5, y0, y1, y2, y3, y4 in test_dataloader:
                ...
                outputs1 = model1(inputs1_t1, inputs3_fa)
...
if __name__ == '__main__':
    ...
    ##
    model1 = MMFNet(1,1,5).to(device)
    model1 = nn.DataParallel(model1).cuda()
    folds = ['fold1', 'fold2', 'fold3', 'fold4', 'fold5']
    for fold in folds:
        pre_file_t1_name = '/media/brainplan/XLdata/CNTSeg++/Predict/T1_FA/MMFNet/' + fold + '/test_val'
        ...
        weight_path = '/media/brainplan/XLdata/CNTSeg++/Weights/T1_FA/MMFNet/' + fold
        ...
        for weight in tqdm(weights[:], ncols=100):
            weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1, test_imgs_path, fold)
            sleep(0.01)
    end_time = time.time()
