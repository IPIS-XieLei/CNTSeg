#import visdom
import config_2d
from NetModel import Unet_2d,NSYnet
import torch, time
import math
...
def train(weights_out_path,fold):
    ...
    # 模型载入
    # model.load_state_dict(torch.load('/media/brainplan/XLdata/CNTSeg++/Weights/T1_T2/FECC/CN_37_epoch_64_batch.pth', map_location='cuda'))
...
#### start train
    ...
    for epoch in range(0, 150):
        ...
        for x1, x2, x3, x4, x5, y0, y1, y2, y3, y4 in train_dataloader:
            ...
            # 梯度清零
            optimizer.zero_grad()
            outputs = model(inputs1_t1, inputs4_dec)
...
if __name__ == '__main__':
    ...
    for fold in folds:
        print('/media/brainplan/XLdata/CNTSeg++/Weights/NSYnet/' + fold)
        weights_out_path = '/media/brainplan/XLdata/CNTSeg++/Weights/NSYnet/' + fold
