...
def weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1,test_imgs_path, mask_idx, fold):  #
...
if __name__ == '__main__':
    ...
    ##
    mask_idx = 15
    model0 = Model(1,1,1,3,9,5).to(device)
    model1 = nn.DataParallel(model0).cuda()
    folds = ['fold1', 'fold2', 'fold3', 'fold4', 'fold5']
    for fold in folds:
        pre_file_t1_name = '/media/brainplan/XLdata/CNTSeg++/Predict/CNTSegV2/CNTSegV2_no_dedicated_without_SDM/' + fold + '/test_val'
        ...
        weight_path = '/media/brainplan/XLdata/CNTSeg++/Weights/CNTSegV2/CNTSegV2_no_dedicated_without_SDM/' + fold
        ...
        for weight in tqdm(weights[:], ncols=100):
            weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1, test_imgs_path, mask_idx, fold)
            sleep(0.01)
    end_time = time.time()
