...
def weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1,test_imgs_path):  #
...
if __name__ == '__main__':
    ...
    ##
    model1 = unet2d(3,5).to(device)
    model1 = nn.DataParallel(model1).cuda()
    ##
    folds = ['fold1', 'fold2', 'fold3', 'fold4', 'fold5']
    for fold in folds:
        pre_file_t1_name = '/media/brainplan/XLdata/CNTSeg++/Predict/Single/Predict_DEC/' + fold + '/test_val'
        ...
        weight_path = '/media/brainplan/XLdata/CNTSeg++/Weights/Single/' + fold + '/outputs_CN_DEC'
        ...
        for weight in tqdm(weights[:],ncols = 100):
            weights_list(weight, pre_file_t1_name, test_data_name, input_label_base, model1,test_imgs_path)
            sleep(0.01)
    end_time = time.time()
