...
Model = CNTSegV2_final.CNTSegV2_NO_Dedicated_without_ALL
batch_size = 64
...
def train(weights_out_path, fold):
    ...
    # 模型载入
    model.load_state_dict(torch.load('/media/brainplan/XLdata/CNTSeg++/Weights/CNTSegV2/CNTSegV2_no_dedicated_without_ALL/fold5/CN_140_epoch_64_batch.pth', map_location='cuda'))
...
#### start train
    ...
    for epoch in range(140, 200):
        ...
        for x1, x2, x3, x4, x5, y0, y1, y2, y3, y4, mask in train_dataloader:
            ...
            # 梯度清零
            optimizer.zero_grad()
            outputs= model(inputs1_t1,inputs2_t2,inputs3_fa,inputs4_dec,inputs5_peaks,modality_mask)
            #####################################################
...
if __name__ == '__main__':
    ...
    for fold in folds:
        print('/media/brainplan/XLdata/CNTSeg++/Weights/CNTSegV2/CNTSegV2_no_dedicated_without_ALL/' + fold)
        weights_out_path = '/media/brainplan/XLdata/CNTSeg++/Weights/CNTSegV2/CNTSegV2_no_dedicated_without_ALL/' + fold
