...
class UNet2D(nn.Module):
    ...
    def forward(self, inputs):                  #  in->out       1*512*512      1*64*64
        ...
        # output = torch.where(final > 0.5, final, torch.tensor([0.0]))
        return final
...
class DatafusionNet_4(nn.Module):
    def __init__(self, in_channels_1=4, n_classes=2, is_deconv=True, is_batchnorm=True):
        super(DatafusionNet_4, self).__init__()
    ...
    def forward(self, inputs1, inputs2, inputs3, inputs4):
        intput = torch.cat([inputs1, inputs2, inputs3, inputs4], 1)
        conv3x3 = self.Conv2d3x3(intput)  #4 ->128
        maxpool_1 = self.maxpool(conv3x3)   # 128->128
        conv3x3_2 = self.Conv2d3x3_2(maxpool_1)
        up2d2x2 = self.upConv2d2x2(conv3x3_2) # 128
...
if __name__ == '__main__':
    ...
    print('#### Test Case ###')
    model = CNTSegV2_BaselineSE_SM_FIM_SMFF(1,1,1,3,9,5).to(device)
