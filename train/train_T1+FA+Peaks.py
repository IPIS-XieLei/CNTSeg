...
def train():
    ...
    # 验证集选择
    # # For model 4 (CTL_t1+fa)
    # CN_val_x_t1_dir = 'CN_mydata/val_T1_FA/x_t1_data/'
    # CN_val_x_fa_dir = 'CN_mydata/val_T1_FA/x_fa_data/'
    # CN_val_x_peaks_dir = 'CN_mydata/val_T1_FA/x_peaks_data/'
    # CN_val_y_on_dir = 'CN_mydata/val_T1_FA/y_data_on/'
    # CN_val_y_tgn_dir = 'CN_mydata/val_T1_FA/y_data_tgn/'
    # CN_val_y_fvn_dir = 'CN_mydata/val_T1_FA/y_data_fvn/'
    # CN_val_y_ocn_dir = 'CN_mydata/val_T1_FA/y_data_ocn/'
...
#### start train
    ...
    for epoch in range(0, 500):
        ...
        model_path = 'outputs_CN_T1+FA+Peaks/' + 'CN_%d_epoch_%d_batch.pth' % (epoch + 1, batch_size)
        ...
        # model.eval()
        # with torch.no_grad():
        #     for x1, x2, x3, y0, y1, y2, y3, y4 in val_dataloader:
        #         step_val += 1
        #         inputs1 = x1.to(device)  # [batch_size, 9, 144, 144]->model(9,2)-> output:[batch_size, 2, 144, 144]
        #         # inputs2 = x2.to(device)
        #         groundtruth0 = y0.to(device)
        ...
        #         mean_dice = (on_dice + tgn_dice + fvn_dice + ocn_dice) / 4
        #         #################################################
        #         t1_pred_result = output.squeeze().cpu().numpy()
        ...
        #         ############################################
        #         print(
        #             "epoch:%d/%d, %d/%d, Val Mean Dice :%0.3f, on_dice :%0.3f, tgn_dice :%0.3f, fvn_dice :%0.3f, ocn_dice :%0.3f" % (
        #             epoch + 1,
        #             n_epochs,
        #             step_val * val_dataloader.batch_size,
        #             dt_size, mean_dice, on_dice, tgn_dice, fvn_dice, ocn_dice))
        #
...
if __name__ == '__main__':
    ...
    # 模型保存
    if 'outputs_CN_T1+FA+Peaks' not in os.listdir(os.curdir):
        os.mkdir('outputs_CN_T1+FA+Peaks')
