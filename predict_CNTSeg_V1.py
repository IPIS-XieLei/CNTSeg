...
# fusionnet = Newnet.DatafusionNet_3
# segmentation_fusionnet = cntseg_model.segmentation_net
...
# test_imgs_path = '/media/brainplan/XLdata/CNTSeg++/Data/NewDataset/TestValSet'
...
def weights_list(weight, pre_file_t1_name, test_data_name, input_label_base,test_imgs_path,fold,weights_T1,weights_FA,weights_Peaks):  #
    ...
    for test_num in test_dir:
        ...
        # print(
        #     "/media/brainplan/XLdata/CNTSeg++/Weights/Single/" + fold + "/outputs_CN_T1/CN_" + weights_T1 + "_epoch_64_batch.pth")
        # print(
        #     "/media/brainplan/XLdata/CNTSeg++/Weights/Single/" + fold + "/outputs_CN_T1/CN_" + weights_T2 + "_epoch_64_batch.pth")
        # 模型载入
        ...
        model_peak.load_state_dict(
            torch.load("/media/brainplan/XLdata/CNTSeg++/Weights/Single/" + fold + "/outputs_CN_Peaks/CN_" + weights_Peaks + "_epoch_64_batch.pth",
                       map_location='cuda'))
        model_fusion.load_state_dict(torch.load(model1_path, map_location='cuda'))
        # model1.load_state_dict(torch.load("/home/xielei/CNs/weights/fold1/outputs_CN_cntseg/fold1/CN_40_epoch_64_batch.pth", map_location='cuda'))
        ...
        test_dataset = CN_MyTrainDataset(CN_test_x_t1_dir, CN_test_x_t2_dir, CN_test_x_fa_dir, CN_test_x_dec_dir,
                                         CN_test_x_peaks_dir, CN_test_y_1_dir,
                                         CN_test_y_2_dir, CN_test_y_3_dir, CN_test_y_4_dir,
        ...
        with torch.no_grad():
            for x1, x2, x3,x4,x5, y0, y1, y2, y3, y4 in test_dataloader:
                ...
                outputs1 = model_fusion(outputs_t1,outputs_fa,outputs_peak)
        ...
        for iSlice in range(0, X[2]):
            if np.count_nonzero(img_label_data[:, :, iSlice]) > 0 and np.count_nonzero(
                    img_mask_data[:, :, iSlice]) > 0 and np.count_nonzero(img_data[:, :, iSlice]) > 0:
...
if __name__ == '__main__':
    ...
    for fold in folds:
        i = i + 1
        pre_file_t1_name = '/media/brainplan/XLdata/CNTSeg++/Predict/T1_FA_Peaks/CNTSeg/' + fold + '/test_val'
        ...
        weight_path = '/media/brainplan/XLdata/CNTSeg++/Weights/T1_FA_Peaks/CNTSeg/' + fold
        ...
        for weight in tqdm(weights[:], ncols=100):
            weights_list(weight, pre_file_t1_name, test_data_name, input_label_base,  test_imgs_path, fold,weights_T1[i-1],weights_FA[i-1],weights_Peaks[i-1])
            sleep(0.01)
    end_time = time.time()
    # model_fusion = cntsegfusionnet(15, 5).to(device)
    # # model1 = unet2d(2, 5).to(device)
    # model_fusion = nn.DataParallel(model_fusion).cuda()
    ##
    # pre_file_t1_name = '/media/brainplan/XLdata/CNTSeg++/Predict/T1_T2/CNTSeg/test_val'
    # test_data_name = '/media/brainplan/XLdata/CNTSeg++/CodeNew/CN_mydata/test_val_data'
    # input_label_base = '/media/brainplan/XLdata/CNTSeg++/Data/NewDataset/TestValSet'
    # weight_path = '/media/brainplan/XLdata/CNTSeg++/Weights/T1_T2/CNTSeg'
    # weights = os.listdir(weight_path)
    # for weight in tqdm(weights[:],ncols = 100):
    #     weights_list(weight, pre_file_t1_name, test_data_name, input_label_base)
    #     sleep(0.01)
    # end_time = time.time()
