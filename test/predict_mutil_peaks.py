...
def predict(img_dir, predict_t1, imgs_num):
    ...
    model1_path = 'outputs_CN_Peaks/CN_500_epoch_64_batch.pth'



    # 模型选择
    model1 = unet2d(9, 5).to(device)
    ...
    with torch.no_grad():
        for x1, x2, x3, y0, y1, y2, y3, y4 in test_dataloader:
            ...
            outputs1 = model1(inputs3)
            outputs1 = torch.sigmoid(outputs1)
            outputs1[outputs1 < 0.3] = 0
            outputs1[outputs1 > 0.3] = 1
...
if __name__ == '__main__':
    ...
    # 预测结果保存
    pre_file_t1 = 'predict_peak'  # model1
