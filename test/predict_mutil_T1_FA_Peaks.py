...
def predict(img_dir, predict_t1, imgs_num):
    ...
    # model1_path = 'outputs_CN_T1/CN_500_epoch_64_batch.pth'



    # 模型选择
    model_t1 = unet2d(1, 5).to(device)
    model_fa = unet2d(1, 5).to(device)
    model_peak = unet2d(9, 5).to(device)
    model_fusion = fusionnet(15, 5).to(device)
    ...
    model_fa.load_state_dict(torch.load('outputs_CN_FA/CN_500_epoch_64_batch.pth', map_location='cuda'))
    model_peak.load_state_dict(torch.load('outputs_CN_Peaks/CN_500_epoch_64_batch.pth', map_location='cuda'))
    model_fusion.load_state_dict(torch.load('outputs_CN_T1_FA_Peaks/CN_500_epoch_64_batch.pth', map_location='cuda'))
    ...
    with torch.no_grad():
        for x1, x2, x3, y0, y1, y2, y3, y4 in test_dataloader:
            ...
            outputs_t1 = model_t1(inputs1)  # FA
            outputs_fa = model_fa(inputs2)  # FA
            outputs_peak = model_peak(inputs3)  # FA
            # outputs_t1 = torch.sigmoid(outputs_t1)
            # outputs_peak = torch.sigmoid(outputs_peak)
            outputs_fusion = model_fusion(outputs_t1, outputs_fa, outputs_peak)  # FA
            outputs1 = torch.sigmoid(outputs_fusion)
...
if __name__ == '__main__':
    ...
    # 预测结果保存
    pre_file_t1 = 'predict_T1_FA_Peaks'  # model1
